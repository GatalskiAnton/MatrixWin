...
struct  Params
{
	...
	Params(const std::vector<std::vector<int>>& m1, const std::vector<std::vector<int>>& m2, std::vector<std::vector<int>>& resM, int blockI, int blockJ, int matrixSize, int blockSize) 
		: m1(m1), m2(m2), resM(resM), blockI(blockI), blockJ(blockJ), matrixSize(matrixSize), blockSize(blockSize)
	{
	}

	const std::vector<std::vector<int>>& m1;
	const std::vector<std::vector<int>>& m2;
	std::vector<std::vector<int>>& resM;
	int blockI;
	int blockJ;
	int matrixSize;
	int blockSize;
...
DWORD WINAPI blocksMul (LPVOID params)
{
	Params* paramsPtr = (Params*)params;
	blocksMul(paramsPtr->m1, paramsPtr->m2, paramsPtr->resM, paramsPtr->blockI, paramsPtr->blockJ, paramsPtr->matrixSize, paramsPtr->blockSize);
	ExitThread(0);
}
...
void threadMul(const std::vector <std::vector <int>>& m1, const std::vector <std::vector <int>>& m2, std::vector < std::vector <int>>& resM, int matrixSize, int blockSize)
{
	...
	for (int blockI = 0; blockI < matrixSize; blockI += blockSize)
		...
		{
			g_lock.lock();
			threads.emplace_back(CreateThread(NULL, 0, &blocksMul, new Params(m1, m2, resM, blockI, blockJ, matrixSize, blockSize),0,NULL));
			g_lock.unlock();
		}
